...
float Color::hueCorrection(unsigned _h)
{
...
std::vector< std::pair<int, int> > Lightbox::extractHS(std::vector<Lightbox::Color> const& _c)
{
	std::vector< std::pair<int, int> > ret(_c.size());
	for (unsigned i = 0; i < _c.size(); ++i)
		ret[i] = std::pair<int, int>(_c[i].h(), _c[i].s());
	return ret;
}
